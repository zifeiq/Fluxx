...
private:
	...
	const Card* HandlimitRule;
	int cntKeeperlimit;//所有物上限
	const Card* KeeperlimitRule;
	bool orderReverse;//出牌方向
	const Card* c_orderReverse;
	bool doubleGoals;//双重目标
	const Card* c_doubleGoals;
	const Card* firstGoal;
	const Card* secondGoal;
	bool richBonus;//富人奖励
	const Card* c_richbonus;
	bool poorBonus;//穷人奖励
	const Card* c_poorBonus;
	bool Inflation;//通货膨胀
	const Card* c_Inflation;
